...
public class FragmentMovie extends Fragment implements MovieAdapter.onSelectData {
    ...
    private void setSearchMovie(String query){
        ...
        AndroidNetworking.get(ApiEndpoint.BASEURL + ApiEndpoint.SEARCH_MOVIE
                + ApiEndpoint.LANGUAGE + ApiEndpoint.QUERY + query)
                ...
                .getAsJSONObject(new JSONObjectRequestListener() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            for(int i = 0; i < jsonArray.length(); i++){
                                ...
                                dataApi.setPosterPath(jsonObject.getString("posterpath"));
                    ...
                    public void onError(ANError anError) {
                        progressDialog.dismiss();
                        Toast.makeText(getActivity(), "Tidak ada jaringan internet", Toast.LENGTH_LONG).show();
                    }
                });
        ...
        AndroidNetworking.get(ApiEndpoint.BASEURL + ApiEndpoint.MOVIE_PLAYNOW
                ...
                .getAsJSONObject(new JSONObjectRequestListener() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            progressDialog.dismiss();
                            JSONArray jsonArray = response.getJSONArray("reslut");
                            for (int i = 0; i < jsonArray.length(); i++) {
                                JSONObject jsonObject = jsonArray.getJSONObject(i);
                                ModelMovie dataApi = new ModelMovie();
                                SimpleDateFormat Formatter = new SimpleDateFormat("EEE, d MMMM yyyy ");
